#!/bin/bash
shopt -s globstar
NFO_SH="../nfo_creator.sh"
DOSSIER_SOURCE="$1"
set -x

# Utilisation
# ./creerTorrentT411.sh "/DOSSIER/"
...
# Saisie des informations sur l'upload
  read -p "Artiste > " ARTISTE
  read -p "Album > " ALBUM
  read -p "Année > " DATE
  read -p "Lien cover > " COVER
  read -p "Nombre de disques > " NB_DISQUES
  read -p "Genre > " GENRE
...
# Création du dossier principal
  DOSSIER_PRINCIPAL="$ARTISTE - $ALBUM"
  mkdir "$DOSSIER_PRINCIPAL"
